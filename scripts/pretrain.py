...
if __name__ == '__main__':
    ...
    model = models.__dict__[args.arch](
        pretrained=args.pretrained, num_classes=n_class)
    print("=> creating model '{}'".format(args.arch),
          ' pretrained is ', args.pretrained)
    ...
    if args.half:
        ...
        model, optimizer = apex.amp.initialize(
            model, optimizer, opt_level=args.half_type)
    ...
    if args.resume:
        ...
        if os.path.isfile(os.path.join(args.checkpoint, 'log.txt')):
            ...
            logger = Logger(os.path.join(
                args.checkpoint, 'log.txt'), title=title)
